...
def getROI(image):
    ...
    trapezoid = np.array([[(0, height * 5//6), 
                          (width, height * 5//6), 
                          (width * 2//3, int(height//2)),
    ...
    mask = cv2.fillPoly(black_image, trapezoid, 255)
    # applying mask on original image
    masked_image = cv2.bitwise_and(image, mask)
    return masked_image

def getLines(image):
    lines = cv2.HoughLinesP(image, 0.3, np.pi/180, 100, np.array([]), minLineLength=5, maxLineGap=10)
    return lines
...
image = cv2.imread("test_images/4.png") #Load Image
cv2.imshow("ROI", getROI(image))
cv2.waitKey(0)
